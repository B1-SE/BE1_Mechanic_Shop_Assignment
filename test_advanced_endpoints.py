...
def test_advanced_endpoints():
    ...
    # Create customers
    customers_data = [
        {"first_name": "Alice", "last_name": "Smith", "email": "alice.smith@test.com", "phone_number": "555-0001", "password": "password123"},
        {"first_name": "Bob", "last_name": "Johnson", "email": "bob.johnson@test.com", "phone_number": "555-0002", "password": "password123"},
        {"first_name": "Charlie", "last_name": "Brown", "email": "charlie.brown@test.com", "phone_number": "555-0003", "password": "password123"},
        {"first_name": "Diana", "last_name": "Adams", "email": "diana.adams@test.com", "phone_number": "555-0004", "password": "password123"},
        {"first_name": "Eve", "last_name": "Wilson", "email": "eve.wilson@test.com", "phone_number": "555-0005", "password": "password123"}
    ]
    ...
    # Create mechanics
    mechanics_data = [
        {"name": "John Wrench", "email": "john.wrench@shop.com", "phone_number": "555-1001", "hourly_rate": 45.0},
        {"name": "Jane Gear", "email": "jane.gear@shop.com", "phone_number": "555-1002", "hourly_rate": 50.0},
        {"name": "Mike Bolt", "email": "mike.bolt@shop.com", "phone_number": "555-1003", "hourly_rate": 40.0}
    ]
    ...
    service_tickets_data = [
        {"customer_id": customer_ids[0], "description": "Oil change and tire rotation", "service_date": "2025-09-21", "mechanic_ids": [mechanic_ids[0]]},
        {"customer_id": customer_ids[1], "description": "Brake inspection", "service_date": "2025-09-22", "mechanic_ids": [mechanic_ids[0], mechanic_ids[1]]},
        {"customer_id": customer_ids[2], "description": "Engine diagnostic", "service_date": "2025-09-23", "mechanic_ids": [mechanic_ids[1]]},
        {"customer_id": customer_ids[0], "description": "Transmission service", "service_date": "2025-09-24", "mechanic_ids": [mechanic_ids[0], mechanic_ids[2]]}
    ]
    ...
    response = requests.get(f"{BASE_URL}/customers/?search=a&sort_by=last_name&order=asc&page=1&per_page=2")
    if response.status_code == 200:
        data = response.json()
        print(f"✅ Combined query: {len(data['customers'])} customers with 'a', sorted by last_name")
    ...
    if ticket_ids:
        ...
        if response.status_code in [400, 207]:  # Bad request or multi-status
            data = response.json()
            if 'errors' in data:
                print(f"✅ Bulk edit error handling: {len(data['errors'])} errors reported")
            else:
    ...
    print("✨ All advanced features (pagination, sorting, filtering, bulk operations) tested!")
