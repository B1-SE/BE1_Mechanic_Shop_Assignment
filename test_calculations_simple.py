...
def test_calculations_endpoints():
    ...
    # Test Addition
    print("\n➕ Testing Addition:")
    response = requests.post(f"{base_url}/calculations/add", 
                           json={'numbers': [2, 3, 5]})
    ...
    # Test Subtraction
    print("\n➖ Testing Subtraction:")
    response = requests.post(f"{base_url}/calculations/subtract", 
                           json={'numbers': [20, 5, 3]})
    if response.status_code == 200:
        data = response.json()
    ...
    response = requests.post(f"{base_url}/calculations/multiply", 
                           json={'numbers': [4, 5, 2]})
    if response.status_code == 200:
        data = response.json()
    ...
    # Test Division
    print("\n➗ Testing Division:")
    response = requests.post(f"{base_url}/calculations/divide", 
                           json={'numbers': [100, 5, 2]})
    if response.status_code == 200:
        data = response.json()
