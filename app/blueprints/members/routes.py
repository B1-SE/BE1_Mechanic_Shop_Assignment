...
from .schemas import (member_create_schema, member_update_schema, member_response_schema, 
...
def create_member():
    ...
    try:
        ...
        if existing_member:
            return jsonify({"error": "Email already associated with another member"}), 400
...
def get_members():
    ...
    try:
        ...
        if is_active_filter is not None:
            response_data['filtered_by_active_status'] = is_active_filter.lower() == 'true'
...
def get_members_by_role(role):
    ...
    try:
        ...
        return jsonify({'error': 'Failed to retrieve members by role', 'message': str(e)}), 500