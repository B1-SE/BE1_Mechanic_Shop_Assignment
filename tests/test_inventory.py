...
class TestInventoryAPI:
    ...
    def test_create_inventory_item_success(self, client, init_database):
        ...
        assert response.status_code in [201, 400, 401]  # API might have specific validation rules
    ...
    def test_create_inventory_item_missing_required_field(self, client, init_database):
        ...
        response = client.post('/inventory/',
                             json=item_data,
                             content_type='application/json')
        assert response.status_code in [400, 401]  # Should fail due to missing fields or auth
    ...
    def test_create_inventory_item_negative_quantity(self, client, init_database):
        ...
        response = client.post('/inventory/',
                             json=item_data,
                             content_type='application/json')
        assert response.status_code in [400, 401]  # Should fail due to validation or auth
    ...
    def test_update_inventory_item_not_found(self, client, init_database):
        ...
        response = client.put('/inventory/999',
                            json=update_data,
                            content_type='application/json')
